# NFEP v0.1.1 Sprint 2: Installation System
...
2. **CMake Config File Generation**
   - Create `NFEPConfig.cmake.in` template
   - Generate `NFEPConfig.cmake` with find_package() support
   - Include version information and component detection
   - Install config files to `${CMAKE_INSTALL_PREFIX}/lib/cmake/NFEP`
...
- NFEPConfig.cmake.in template file
...
- `find_package(NFEP)` works after installation
...
3. **Integration Testing**
   - Test `find_package(NFEP)` after installation
...
- Support for `find_package(NFEP)` functionality
