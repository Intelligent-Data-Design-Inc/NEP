...
NEP uses the Virtual Object Layer (VOL) connector architecture of HDF5 to provide transparent access to different file formats. The architecture consists of:
...
```
src/
├── bufr_vol_connector.c     # BUFR connector → libnep_bufr_vol.so
├── bufr_vol_connector.h     # (requires NCEPLIBS-bufr)
├── cdf_vol_connector.c      # CDF connector → libnep_cdf_vol.so
├── cdf_vol_connector.h      # (requires NASA CDF library)
├── grib2_vol_connector.c    # GRIB2 connector → libnep_grib2_vol.so
├── grib2_vol_connector.h    # (requires NCEPLIBS-g2)
├── geotiff_vol_connector.c  # GeoTIFF connector → libnep_geotiff_vol.so
