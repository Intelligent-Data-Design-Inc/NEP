/**
 * @file
 * @brief PDBx/mmCIF dispatch layer.
 *
 ...
 * functions that make up the PDBx/mmCIF dispatch interface. */
...
const NC_Dispatch *MMCIF_dispatch_table = NULL;

/**
 * @internal Initialize the PDBx/mmCIF dispatch layer.
 *
 * Registers the mmCIF handler via `nc_def_user_format()` for direct
 ...
 *
 * @return Pointer to mmCIF dispatch table, or NULL on failure.
 * @author Edward Hartnett
 */
NC_Dispatch*
NC_MMCIF_initialize(void)
{
    ...
    nc_def_user_format(NEP_UDF_MMCIF, (NC_Dispatch *)MMCIF_dispatch_table,
                       NEP_MAGIC_MMCIF);
    return (NC_Dispatch*)&MMCIF_dispatcher;
}

/**
 * @internal Finalize the PDBx/mmCIF dispatch layer.
 *
