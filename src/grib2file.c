...
/*     /\* Allocate data to hold GRIB2 specific file data. *\/ */
/*     if (!(grib2_file = malloc(sizeof(NC_GRIB2_FILE_INFO_T)))) */
/*         return NC_ENOMEM; */
/*     h5->format_file_info = grib2_file; */
/*     grib2_file->sdid = sdid; */

/*     /\* Read the global atts. *\/ */
/*     for (a = 0; a < num_gatts; a++) */
/*         if ((retval = grib2_read_att(h5, NULL, a))) */
/*             break; */

/*     /\* Read each dataset. *\/ */
/*     if (!retval) */
/*         for (v = 0; v < num_datasets; v++) */
/*             if ((retval = grib2_read_var(h5, v))) */
/*                 break; */
...
/**
 * @internal Abort (close) the GRIB2 file.
 *
 * @param ncid File ID.
 *
 * @return NC_NOERR No error.
 * @return NC_EBADID Bad ncid.
 * @return NC_EHDFERR Error from GRIB2 layer.
 * @author Ed Hartnett
 */
int
...
NC_GRIB2_close(int ncid, void *ignore)
{
    ...
    /* /\* Clean up GRIB2 specific allocations. *\/ */
    /* if ((retval = grib2_rec_grp_del(h5->root_grp))) */
    /*     return retval; */

    /* /\* Close grib2 file and free GRIB2 file info. *\/ */
    /* grib2_file = (NC_GRIB2_FILE_INFO_T *)h5->format_file_info; */
    /* if (SDend(grib2_file->sdid)) */
    /*     return NC_EHDFERR; */
    /* free(grib2_file); */
